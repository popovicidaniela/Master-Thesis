...
\begin{abstract}
	\thispagestyle{empty}
	Reinforcement learning is in these days improving rapidly with the new ideas from deep learning. This thesis gives a clear explanation of the theory in reinforcement learning. State of the art methods in deep reinforcement learning is explained, implemented and tested. We present a A3C method which can learn an agent how to control a car in a car simulation environment. It is a lightweight framework because it can be trained on a single multi-core CPU instead of a GPU. The input used in our A3C method is only the raw image from the car simulator, and the output are discrete actions to control the car. The implemented A3C method is optimized by trying different test scenarios. The trained agent can drive without interrupting with obstacles in the environment.     
\end{abstract}
\newpage
\mbox{}
\thispagestyle{empty}
