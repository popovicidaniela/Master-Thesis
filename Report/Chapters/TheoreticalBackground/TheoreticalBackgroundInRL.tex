...
The \textit{state-value function} for policy $\pi$, $v_{\pi }$ is the following:

The \textit{action-value function} for policy $\pi$, $q_{\pi }$ is the following:

Value functions have the property of being expressed recursively. The recursive representation is actually the Bellman equation and it's solution is the value of $v_{\pi }$:
