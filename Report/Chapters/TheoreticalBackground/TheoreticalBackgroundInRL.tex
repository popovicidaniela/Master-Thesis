...
The Bellman optimality equation generates a system of $N$ nonlinear equations, where $N$ is the number of states. It can be simply be solved by applying some nonlinear methods when the system dynamics ($p(s',r|s,a)$) are known. Given the Bellman optimality equation solution one has the optimal policy, e.g. one can, at any state, choose the action that corresponds to the maximum return, which is also valid in the long term, because the values take into account the reward consequences of all possible future behavior \cite{Sutton}. In the case of the action-value pairs the system's dynamics might be unknown and the actions would still be optimal, because it would choose the actions that would maximize $q_{*}$.
