...
\end{figure}
As we can see from the \Cref{fig:feedforward} the feedforward ANNs do not have loops. The \textit{recurrent} ANNs have loops instead, and are also used in RL. A very important type of ANN is the \textit{convolutional} one, which is especially used for image processing and RL. 

Some ANNs can be called \textit{deep}. This qualifier describes the networks that have many hidden layers, precisely more than two. Deep ANNs are harder to train, but they are more powerful, and are especially used in modern artificial intelligence applications.
